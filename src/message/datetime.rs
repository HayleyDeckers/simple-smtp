...
impl DateTime {
    ...
    pub fn from_timestamp(secs: i64) -> Option<Self> {
        ChronoDateTime::<Utc>::from_timestamp(secs, 0)
            .map(|dt: ChronoDateTime<Utc>| DateTime(dt.fixed_offset()))
    }
    ...
    pub fn to_zone(self, zone: TimeZone) -> Option<Self> {
        let offset = FixedOffset::east_opt(zone.offset_seconds())?;
        // Convert to UTC first, then to target timezone (keeps same point in time)
        let utc = self.0.with_timezone(&Utc);
        Some(DateTime(utc.with_timezone(&offset)))
    }
    ...
    pub fn now_local(zone: TimeZone) -> Self {
        let utc = Utc::now();
        let offset =
            FixedOffset::east_opt(zone.offset_seconds()).expect("zone offset should be valid");
        DateTime(utc.with_timezone(&offset))
    }
}
...
impl fmt::Display for DateTime {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        // FixedOffset::local_minus_utc() returns seconds, positive means local is ahead of UTC
        // RFC 5322 format: +HHMM means local is ahead of UTC (same convention)
        // So we use it directly without negation
        ...
        // offset_seconds is positive when local is ahead of UTC (e.g., UTC+5:30)
        let offset_abs = offset_seconds.abs();
        let offset_hours = (offset_abs / 3600) as u32;
        let offset_minutes = ((offset_abs % 3600) / 60) as u32;
